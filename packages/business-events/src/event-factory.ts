import { SQSRecord, APIGatewayProxyEvent } from 'aws-lambda';
...
export class EventFactory {
   ...
   */
  static fromSqsRecord<T extends EventFactory>(
    this: new (id: string) => T,
    record: Pick<SQSRecord, 'body'>
  ): T {
    const id = JSON.parse(record.body).correlationId;
    return new this(id);
  }
  ...
  static fromApiProxyEvent<T extends EventFactory>(
    ...
    const id = event.headers['X-Correlation-ID'];
    return new this(id);
  }
