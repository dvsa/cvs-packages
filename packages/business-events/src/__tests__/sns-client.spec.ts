...
describe('BusinessEvents', () => {
  ...
  beforeEach(() => {
    delete process.env.USE_CREDENTIALS;
  ...
  describe('create', () => {
    it('creates an extended factory event from an sqs event', async () => {
      const event =
        TestSystemEventFactory.fromSqsRecord(sqsRecord).validDomainEvent('custom-attribute');
    ...
    it('creates an extended factory event from an api gateway proxy event', async () => {
      const event =
        TestSystemEventFactory.fromApiProxyEvent(apiEvent).validDomainEvent('custom-attribute');
  ...
  describe('publish', () => {
    ...
    it('logs an error when publishing without a topic arn', async () => {
      const invalidPayload =
        TestSystemEventFactory.fromSqsRecord(sqsRecord).invalidEventMissingArn();

      await expect(BusinessEventPublisher.publish(invalidPayload));
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        new Error('Business event requires a topic ARN')
      );
    });

    it('logs an error when publishing without an event name', async () => {
      const invalidPayload =
        TestSystemEventFactory.fromSqsRecord(sqsRecord).invalidEventMissingName();

      await expect(BusinessEventPublisher.publish(invalidPayload));
      expect(consoleErrorSpy).toHaveBeenCalledWith(
        new Error('Business event requires an event name')
      );
    });
  });
});
